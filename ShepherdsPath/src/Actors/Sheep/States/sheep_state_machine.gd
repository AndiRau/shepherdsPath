...
func _physics_process(_delta):
	...
	if get_parent().target_pos != Vector3.INF:
		flag_vector = global_transform.origin.direction_to(get_parent().target_pos) * _c_speed * target_follow_force
	...
	if is_jumping:
		if rc.is_colliding():
			...
			_down_force -= jump_shortage
		_down_force = clamp(_down_force, -down_force_max*2, down_force_max)
	else:
		_down_force = -5
